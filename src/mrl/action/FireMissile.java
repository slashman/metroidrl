...
public class FireMissile extends Action{
	...
	public void execute(){
		...
		if (targetPosition.equals(performer.getPosition())){
			aLevel.addMessage("You shot you missile upside");
			return;
        }
		...
		Effect me = EffectFactory.getSingleton().createDirectedEffect(performer.getPosition(), targetPosition, "MISSILE", i);
		aLevel.addEffect(me);
		...
		for (i=0; i<20; i++){
			...
			if (
				...
				int penalty = 0;
				int attack = player.getMissileAttack();
				attack -= penalty;
				if (attack < 1)
					attack = 1;
				message = "Your missile hits the "+targetMonster.getDescription();
			...
			if (targetMapCell != null && targetMapCell.getID().equals("MISSILE_DOOR")){
	...
	public boolean canPerform(Actor a){
		...
		if (player.getMissilesCapacity() == 0){
			invalidationMessage = "[/missile.energyMissile {Suit Functionality Unavailable}]";
			return false;
		}
		if (player.getMissiles() <1){
			invalidationMessage = "You are short of missile energy.";
			return false;
		}
